...
def csv_reader(filename):
    data=pd.read_csv(os.path.join(my_path, './cardio/{}.csv').format(filename),names=['time','HR','State'],skiprows=1)
    data.head(5)
...
available_indicators = sort_list([Path(p).stem for p in glob.glob(os.path.join(my_path, './cardio/*.csv'))])
