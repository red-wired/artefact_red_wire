...
def lowpassfilter(df, signal, threshold, wavelet_mode, wavedec_mode, waverec_mode, thresh_mode):
    data = df.copy()
...
def method4(df, threshold):
    thresh_mode = 'soft'
    wavelet_mode = 'db8'
    wavedec_mode = 'sym'
    waverec_mode = 'sym'
    signal = df.HR.fillna(np.mean(df.HR)).values # Toutes les valeurs doivent exister
    rec = lowpassfilter(df, signal, threshold, wavelet_mode, wavedec_mode, waverec_mode, thresh_mode)
    return rec

# thresh_mode = ['soft', 'hard', 'garrote', 'greater', /'less']
# wavelet_mode = ['db1' to 'db38', /'haar'] ['haar', 'db', 'sym', 'coif', 'bior', 'rbio', 'dmey', 'gaus', 'mexh', 'morl', 'cgau', 'shan', 'fbsp', 'cmor']
